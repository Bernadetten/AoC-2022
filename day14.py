...
second_part = bool(int(sys.argv[2]))    # Do we compute the answer to part 2 of the puzzle?
...
# Set up empty grid
data = [["." for _ in range(highest_x_coordinate + more_width_right)] for _ in range(highest_y_coordinate+1)]
...
if second_part:
    data += [["." for _ in range(highest_x_coordinate + more_width_right)]] + [["#" for _ in range(highest_x_coordinate + more_width_right)] for _ in range(5)]
