...
def update_location_tail(location_head, location_tail):
    ...
    if (
        abs(location_head[0] - location_tail[0]) > 1
        or abs(location_head[1] - location_tail[1]) > 1
    ):
        best_move = (0, 0)
        best_distance = 100
        for move in possible_moves:
            distance = abs(location_head[0] - (location_tail[0] + move[0])) + abs(
                location_head[1] - (location_tail[1] + move[1])
            )
...
for instruction in instructions:
    ...
    for i, knot in enumerate(location_ropes):
        if i != 0:
            ...
            location_ropes[i] = update_location_tail(
                location_ropes[i - 1], location_ropes[i]
            )
